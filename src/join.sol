...
contract GemJoin2 is LibNote {
    ...
    function transfer(int wad, address guy) private {
        int256 prevBalance = int(gem.balanceOf(address(this)));
        ...
        require(uint(prevBalance + wad) == gem.balanceOf(address(this)), "GemJoin2/failed-transfer");
    }

    function transferFrom(int wad) private {
        int256 prevBalance = int(gem.balanceOf(msg.sender));
        require(prevBalance >= wad, "GemJoin2/no-funds");
        require(int(gem.allowance(msg.sender, address(this))) >= wad, "GemJoin2/no-allowance");
        (bool ok,) = address(gem).call(
            abi.encodeWithSignature("transferFrom(address,address,uint256)", msg.sender, address(this), wad)
        );
        require(ok, "GemJoin2/failed-transfer");
        require(uint(prevBalance - wad) == gem.balanceOf(msg.sender), "GemJoin2/failed-transfer");
    }
...
contract GemJoin3 is LibNote {
    ...
    function deny(address usr) external note auth { wards[usr] = 0; }
    modifier auth {
      require(wards[msg.sender] == 1, "GemJoin3/not-authorized");
      _;
    }
    ...
    function mul(int x, uint y) internal pure returns (int z) {
        require(y == 0 || (z = x * int256(y)) / int256(y) == x, "GemJoin3/overflow");
    }
...
contract GemJoin4 is LibNote {
    ...
    function deny(address usr) external note auth { wards[usr] = 0; }
    modifier auth {
      require(wards[msg.sender] == 1, "GemJoin4/not-authorized");
      _;
    }
